...
def hello_world():
    r = remote("83.136.253.59", 53040)
    r.recvuntil(b"> ")
    return r
def htb_secure_signing_hash(r: remote, s: bytes):
    r.send(b"1\n")
    r.recvuntil(b": ")
    r.send(s+b"\n")
    return r.recvuntil(b"> ").split(b"\n")[0][6:]
def htb_secure_signing_validate(r: remote, s: bytes, hash: bytes):
    r.send(b"2\n")
    r.recvuntil(b": ")
    r.send(s+b"\n")
    r.recvuntil(b": ")
    r.send(hash+b"\n")
    return r.recvuntil(b"> ").split(b"\n")[0]==b"[+] Signature Validated!"
